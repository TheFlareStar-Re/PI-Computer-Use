...
const { runInstall, probe } = require("./cua");
...
async function onLoad() {
  ...
  await pi.commands.register({
    id: COMMAND_ID,
    title: "Computer Use: Open Panel",
    keywords: ["computer use", "cua", "desktop", "自动化", "桌面"],
    run: async () => {
      await pi.ui.openPanel({ title: "Computer Use" });
    },
  });
...
async function onPanelInvoke(channel, payload) {
  ...
  if (channel === "cu.probe") {
    return { ...(await panelState()), probe: probe() };
  }
  if (channel === "cu.install") {
    const installed = await runInstall();
    return { ...(await panelState()), doctor: { text: installed.text }, probe: installed.probe };
  }
