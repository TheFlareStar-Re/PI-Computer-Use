...
const { installPreview } = require("../cua");
...
test("install preview single-quotes executable paths without interpolating special characters", () => {
  const program = "C:\\Program Files\\$Power'Shell\\pwsh.exe";
  const preview = installPreview({ powershellExe: program });
  assert.equal(preview.program, program);
  assert.ok(preview.display.startsWith("& 'C:\\Program Files\\$Power''Shell\\pwsh.exe' -NoProfile"));
  assert.ok(preview.args.some((arg) => arg.includes("https://cua.ai/driver/install.ps1")));
});
