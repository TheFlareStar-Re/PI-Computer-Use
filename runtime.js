...
const { probe: probeCua, doctor: cuaDoctor, installPreview } = require("./cua");
...
class ComputerUseRuntime {
  ...
  binaryPath() {
    const info = probeCua();
    if (!info.installed || !info.mcpCommand) {
      throw new Error("cua-driver not installed. Open the Computer Use panel and install the driver.");
    }
  ...
  start() {
    ...
    this.lastError = "";
    const starting = this._startChild().finally(() => {
  ...
  doctor() {
    return Promise.resolve(cuaDoctor(this.info.exe));
  }
}
